...
<style>
  ...
  /* body {
    ...
    .upload-section .card, .result-section .card {
        flex-grow: 1;
        display: flex;
        flex-direction: column;
    }
    .upload-section .card-body, .result-section .card-body {
        flex-grow: 1;
        display: flex;
        flex-direction: column;
    }
    ...
    uploadForm.addEventListener('submit', async function(e) {
        ...
        simulateLoading(async () => {
            ...
            results.forEach(result => {
                resultHTML += `<li><strong>${result.className}</strong>: ${(result.probability * 100).toFixed(2)}%</li>`;
            ...
            try {
                const response = await fetch(`/waste-category/${encodeURIComponent(highestResult.className)}`);
                ...
                let errorMessage = 'Maaf, terjadi kesalahan saat mengambil informasi tambahan.';
                if (error instanceof TypeError) {
                    errorMessage += ' Format data tidak sesuai.';
                } else if (error instanceof Error && error.message === "Data tidak lengkap") {
                    errorMessage += ' Data yang diterima tidak lengkap.';
                } else if (error.message.startsWith('HTTP error!')) {
                    errorMessage += ` Kode status: ${error.message.split(':')[1].trim()}.`;
                }
                infoContent.innerHTML = `<p>${errorMessage}</p><p>Silakan coba lagi nanti atau hubungi administrator.</p>`;
                infoCard.style.display = 'block';
            }
